<template>
  ...
  <div class="wheel-popover" v-if="showWheel">
    <button class="wheel-popover__button" type="button" @click="spinWheel('cw')">SPIN!</button>
    <div class="wheel">
      <div class="wheel__spinner" v-bind:style="'transform: rotate(' + wheelSpinDegs + 'deg);'">
        <div v-for="(seg, index) in wheelSegments"
          :key="seg + '' + index"
          class="wheel__segment"
          @click.native="definePts(seg)">
  ...
  <div class="board">
    <template v-for="(word, index) in phrase" :key="word + '' + index">
      <span class="board__letter board__letter--space" v-if="index !== 0">&nbsp;</span><span class="board__word">
        <span v-for="(item, index2) in word"
          :key="item + '' + index2"
          class="board__letter"
          v-bind:hidden="!item.visible">{{ item.char }}</span>
      </span>
    </template>
  </div>

  <div class="clue">{{ clue }}</div>
  <div class="text-white text-center font-bold text-3xl">Valendo {{ currentScore }}pts</div>

  <div class="controls">
    <div class="controls__grid">
      <button class="controls__button"
        type="button"
        v-for="(used, letter) in alphabetUsed"
        :key="letter"
        v-bind:disabled="used"
        v-on:click="activateLetter(letter)">{{ letter }}</button>
    </div>
  ...
  <div class="players">
    <section class="text-gray-600 body-font my-10">
      <div class="container px-5 py-10 mx-auto bg-white rounded">
